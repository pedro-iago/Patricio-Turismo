...
public class PessoaController {
    ...
    // (Não temos mais o Repository aqui)
    ...
    @GetMapping("/{idPessoa}")
    public ResponseEntity<Object> getById(@PathVariable(value = "idPessoa") Long idPessoa) { // <-- MUDOU
        Optional<Pessoa> pessoa = service.findById(idPessoa);
        if (pessoa.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Pessoa não existente"); // <-- MUDOU
        }
        return ResponseEntity.status(HttpStatus.OK).body(pessoa.get()); // <-- MUDOU (Status 200 OK)
    }

    @PostMapping
    public ResponseEntity<Pessoa> save(@RequestBody PessoaDto pessoaDto) { // <-- MUDOU
        var pessoaSalva = service.save(pessoaDto);
        return ResponseEntity.status(HttpStatus.CREATED).body(pessoaSalva);
    }

    @DeleteMapping("/{idPessoa}")
    public ResponseEntity<Object> delete(@PathVariable(value = "idPessoa") Long idPessoa) { // <-- MUDOU
        boolean deletado = service.delete(idPessoa);
        if (!deletado) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Pessoa não existente"); // <-- MUDOU
        }
        return ResponseEntity.status(HttpStatus.OK).body("Pessoa deletada com sucesso"); // <-- MUDOU
    }
