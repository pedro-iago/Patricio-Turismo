...
export default function TripDetailsPage() {
    ...
    const handleSelectSeat = (identifier: number, seatNumber: string, isOccupied: boolean) => {
        ...
        if (isOccupied) {
            // === CORREÇÃO AQUI ===
            // Se está ocupado, o 'identifier' que vem do SeatMap JÁ É o ID do passageiro.
            // Não precisamos buscar por número de assento (que dava erro por causa do "02" vs "2").
            const passenger = passengers.find(p => p.id === identifier);
            
            if (passenger) {
                setPassengerToDesassociate(passenger);
            } else {
               // Fallback caso algo estranho aconteça, tenta buscar por assento
               const pLegado = passengers.find(p => p.numeroAssento === seatNumber || parseInt(p.numeroAssento || '0') === parseInt(seatNumber));
               if (pLegado) setPassengerToDesassociate(pLegado);
            }
        } else {
            // Se livre, abre modal para vincular
        ...
        <div className="space-y-4 md:space-y-6 pb-20 md:pb-0">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
                ...
                <div className={cn(
                    ...
                    isMapOpen ? "xl:col-span-9" : "xl:col-span-12", // Lógica de largura
                    mobileView === 'map' ? "hidden xl:block" : "block" // Mobile toggle
                )}>
                ...
                <div className={cn(
                    ...
                    isMapOpen ? "xl:block" : "xl:hidden" // Desktop
                )}>
