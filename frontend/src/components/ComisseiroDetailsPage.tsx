...
// Importe suas funções de API (adaptadas para Comisseiro)
import {
  getComisseiroPassageirosReport,
  getComisseiroEncomendasReport,
} from '../services/api';
...
const ComisseiroDetailsPage = () => {
  ...
  // 3. Chamar a API (MODIFICADO para Comisseiro)
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        const [passageirosData, encomendasData] = await Promise.all([
          getComisseiroPassageirosReport(id, inicio, fim), // <-- MUDANÇA AQUI
          getComisseiroEncomendasReport(id, inicio, fim),  // <-- MUDANÇA AQUI
        ]);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
           <Button variant="ghost" size="icon" onClick={() => navigate('/affiliates')}>
            ...
            <div>
              <h2 className="text-2xl font-semibold">Relatório do Comissário</h2> {/* <-- MUDANÇA AQUI */}
...
export default ComisseiroDetailsPage;